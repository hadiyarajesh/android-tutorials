 ...
 *
 * A [TooltipText] behaves similarly to a [Popup], and will use the position of the parent layout
 * to position itself on screen. Commonly a [TooltipText] will be placed in a [Box] with a sibling
 * that will be used as the 'anchor'. Note that a [TooltipText] by itself will not take up any
 * space in a layout, as the tooltip is displayed in a separate window, on top of other content.
 *
 * The [content] of a [TooltipText] will typically be [Text], as well as custom content.
 *
 * [TooltipText] changes its positioning depending on the available space, always trying to be
...
private const val TooltipTimeout = 10_000L - OutTransitionDuration
